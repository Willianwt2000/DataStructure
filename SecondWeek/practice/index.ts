...
function sumNodes(head: ListNode | null): number {
  if (head == null) return 0;
...
function findMax(head: ListNode | null): number {
   if (head === null) return -Infinity;
  let maxNum: number = head.val;
...
function containsValue(head: ListNode | null, target: number): boolean {
